...
/*:
* @plugindesc v.1.3.0 [MZ] Advanced project testing and various settings. Made with ♥ by DKPlugins
* @author DKPlugins
...
/*:ru
* @plugindesc v.1.3.0 [MZ] Расширенное тестирование проекта и различные настройки. Сделано с ♥ от DKPlugins
* @author DKPlugins
* @url https://dk-plugins.ru
* @target MZ
* @help
    ...
    static async _createStamp() {
        ...
        const processDirectory = async (directory) => {
            ...
            for (const entity of entities) {
                ...
                if (isDirectory) {
                    if (ignoredDirectories.some(dir => dir.getFullPath() === entity.getFullPath())) {
                        continue;
                    }
                }
...
DKTools.IO.Directory = class extends DKTools.IO.Entity {
    ...
    getAll(object) {
        ...
        if (!Utils.isNwjs() && DKTools.IO.mode === DKTools.IO.MODE_NWJS_STAMP) {
            const parts = this.getFullPath().split(DKTools.IO.sep);
...
DKTools.Window.prototype.setupSize = function(width, height) {
    if (DKTools.Utils.isFunction(height)) { // number of lines
